...
export const tabs = (() => {
	...
	const init = (customCfg?: object) => {
		// Run only once
		if (vars.tabs?.init) return; // Change the module name

		vars.tabs = { // Change the module name
			init: false,
			/**
		...
		cfg.tabs = { // Change the module name
			enabled: true
			/**
		...
		if (cfg.tabs.enabled) { // Change the tabs name
			lqx.log('Initializing tabs'); // Change the module name
