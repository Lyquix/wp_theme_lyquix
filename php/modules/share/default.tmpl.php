...
<div class="lqx-module-share">
	<ul class="share-list <?= esc_attr($s['style']) ?>" style="<?= esc_attr(\lqx\modules\share\get_inline_style($s)) ?>">
		<?php foreach ($s['platforms'] as $p) :
			$share_link = \lqx\modules\share\get_share_link($p['platform_name']['value']);
			if ($share_link) : ?>
			<li>
				<a class="link-<?= esc_attr($p['platform_name']['value']) ?>" href="#"
					data-platform="<?= esc_attr($p['platform_name']['value']) ?>"
					data-link="<?= esc_url($share_link) ?>"
	...
	<?php wp_print_inline_script_tag(ob_get_clean(), ['id' => 'lqx-share']); ?>
</div>
